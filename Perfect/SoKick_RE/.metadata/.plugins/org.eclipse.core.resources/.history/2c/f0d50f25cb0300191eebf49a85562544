...
<%
	...
	try {
		...
		findsql = "select * from student where sid = " +sendid;
		...
		if(count == 0)
		{
			sql = "insert into student values('"+sendid+"','"+sendname+"','"+sendpass+"','"+sendphone+"')";
			count = stmt.executeUpdate(sql);
			response.sendRedirect("frontend/login.html");
		}
		else
		{
			response.sendRedirect("frontend/signup.html");
		}
